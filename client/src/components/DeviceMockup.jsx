    ...
    >
      {glow && (
        <div
          ...
          style={{
            background: 'conic-gradient(from 0deg, #FF6B6B, #FFD93D, #6BCB77, #FF6B6B)',
          }}
        />
      )}
      ...
      <div className="relative w-full h-full rounded-[1.5rem] sm:rounded-[2rem] bg-navy shadow-soft border border-white/10 overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[55%] h-[35%] rounded-lg sm:rounded-xl bg-[#1a2940] border-2 border-mint/30 shadow-glow">
          <div className="grid grid-cols-5 grid-rows-3 gap-[2px] sm:gap-[3px] p-1.5 sm:p-2 h-full">
            {Array.from({ length: 15 }).map((_, i) => (
              <div
                ...
                style={{
                  backgroundColor: ['#6BCB77', '#FFD93D', '#FF6B6B', '#6BCB77'][i % 4],
